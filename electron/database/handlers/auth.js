...
function restoreSession(userId) {
  try {
    ...
    if (!user || !user.is_active || user.must_change_password) return { success: false }
    db.prepare("UPDATE users SET is_online = 1, last_seen = datetime('now') WHERE id = ?").run(userId)
    const permissions = db.prepare('SELECT permission_key FROM user_permissions WHERE user_id = ?')
      .all(userId).map(r => r.permission_key)
    const voyageAccess = user.perm_voyage || user.perm_berthing || user.perm_container || user.perm_gc
    const receiptAccess = user.perm_receipt || user.perm_receipt_archive
    return {
      success: true,
      user: {
        ...
        role: user.role, language: user.language, must_change_password: 0,
        permissions,
        perm_storage:         user.perm_storage         ?? 0,
        perm_automate:        user.perm_automate        ?? 0,
        perm_cma:             user.perm_cma             ?? 0,
...
module.exports = { login, changePassword, logout, restoreSession }
